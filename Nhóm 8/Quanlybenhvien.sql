...
--(BẤT KÌ) Liệt kê danh sách bệnh nhân chưa từng được điều trị lần nào (không có trong bảng CHUATRI), kèm loại bệnh nhân và tuổi (tính đến năm 2025).
SELECT 
    BN.TenBenhNhan, 
    BN.LoaiBenhNhan, 
...
--Câu 3: Câu GROUP BY:
--Câu hỏi: Liệt kê tên khu chữa trị và tổng số lượng nhân viên làm việc tại mỗi khu, chỉ hiển thị những khu có từ 2 nhân viên trở lên.
SELECT 
    KC.TenKhuChuaTri, 
    COUNT(L.MaNhanVien) AS SoNhanVien
FROM KHUCHUATRI KC
JOIN LAMVIEC L ON KC.SoKhuChuaTri = L.SoKhuChuaTri
GROUP BY KC.TenKhuChuaTri
HAVING COUNT(L.MaNhanVien) >= 2
--Câu 4: Câu SUBQUERY:
